...
export class GameApp {
    ...
    // Método que associa/vincula os eventos aos elementos HTML
    bindEvents() {
        $('#next').click(() => {
            this.refreshGame();
            this.buildGame(++this.idx);
    ...
    buildGame(x) {
        ...
        // Ao carregar a página, adiciona a classe veryHard (remove sombreamento das letras - css)
        window.onload = () => {
            $models.addClass('veryHard');
            return false;
        };
        ...
        var shuffledLetters = game.word.split( '' ).sort(() => Math.random() < 0.5 ? -1 : 1),
            numberOfRandomLetters = 5;

        for (var i = 0; i < numberOfRandomLetters; i++) {
